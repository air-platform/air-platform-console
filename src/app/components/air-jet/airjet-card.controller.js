...
(function () {
    ...
    angular
        .module('iot')
        .controller('AirjetCardController', AirjetCardController);

    /** @ngInject */
    function AirjetCardController(NetworkService,StorageService, constdata,$state,$rootScope, $uibModal,$log,toastr,i18n, delmodaltip) {
        ...
        function goAddItem() {
            $state.go('app.editairjetcard',{});
        };
        vm.goCopyItem = function (item) {
            $state.go('app.editairjetcard',{username:item.id, args:{type:'copy'}});
        };
        function goEditItem(item) {
            $state.go('app.editairjetcard',{username:item.id, args:{type:'edit'}});
        };

        function goDetail(item) {
            $state.go('app.editairjetcard',{username:item.id, args:{type:'detail'}});
