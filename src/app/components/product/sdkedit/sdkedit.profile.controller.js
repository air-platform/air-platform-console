...
(function () {
    ...
    angular.module('iot').controller('SdkEditProfileCtrl', SdkEditProfileCtrl);

    /** @ngInject */
    function SdkEditProfileCtrl($scope, $uibModal,logger, $log, $state, $stateParams,StorageService, NetworkService, constdata, toastr,UserInfoServer) {
       ...
       // var canvas = new draw2d.Canvas("gfx_holder_profile");
        var figure1 = null;
       ...
       // canvas.uninstallEditPolicy(new draw2d.policy.canvas.DefaultKeyboardPolicy());
        ...
        // console.log(token)
        //
        // console.log('http://10.71.86.222:8888/iot/api/v1/'+constdata.api.product.profileInfoPath+'/'+argsProduct.name+'/profiles/v'+argsSdk.version+'/schemas/profile');
                ...
                function submitData2(savedData) {
                    NetworkService.put(constdata.api.product.profileUpdatePath+'/'+argsProduct.name+'/profiles/v'+argsSdk.version+'/schemas/profile',savedData,function (response) {  
       ...
       // getProData();
        ...
        //console.log(constdata.api.product.profileInfoPath+'/'+argsProduct.name+'/profiles/v'+argsSdk.version+'/schemas/profile');
        /*function getProData() {
            NetworkService.get(constdata.api.product.profileInfoPath+'/'+argsProduct.name+'/profiles/v'+argsSdk.version+'/schemas/profile',null,function (response) {
                //监听数据变化
        ...
        //function createShape(data) {
            /*
            *data是想要生成图形的Object数据
            *设置里可以添加参数，
            *1、设置tableCols为5、6列数据，默认为4列，
            *2、列内容为tabelCellText，类型为字符串组成的数组，
            *不写列内容默认为['请输入']
            *3、可以选择是不是第一个图形，来决定头部是否有输入点，
            *默认带有输入点，如果不需要输入点设置isFirstShape: true
            */
        ...
        */
        ...
        $.ajax({
            url:BASE_API_URL + constdata.api.product.profileInfoPath+'/'+argsProduct.name+'/profiles/v'+argsSdk.version+'/schemas/profile',
            type: 'GET',
