...
(function () {
    ...
    angular
        .module('iot')
        .controller('TraingClassController', TraingClassController);

    /** @ngInject */
    function TraingClassController(NetworkService,StorageService, constdata,$state,$rootScope, $uibModal,$log,toastr,i18n, delmodaltip) {
        ...
        function goAddItem() {
            $state.go('app.edittrainingclass',{});
        };
        vm.goCopyItem = function (item) {
            $state.go('app.edittrainingclass',{username:item.id, args:{type:'copy'}});
        };
        function goEditItem(item) {
            $state.go('app.edittrainingclass',{username:item.id, args:{type:'edit'}});
        };

        function goDetail(item) {
            $state.go('app.edittrainingclass',{username:item.id, args:{type:'detail'}});
